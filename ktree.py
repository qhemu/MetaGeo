...
class KDTree:
    def __init__(self, bucket_size, dimensions, parent=None):
        self.bucket_size = bucket_size  # 50
        self.parent = None
        self.left = None
        self.right = None
    ...
    def findWidestAxis(self):
        widths = [self.max_limit[i] - self.min_limit[i]
    ...
    def nodeSplit(self, cursor, empty_non_leaf=True):
        if cursor.location_count > cursor.bucket_size:
            ...
            # 划分法是最宽维数中所有值的中值
            cursor.split_value = np.median(
                [cursor.index_locations[i][1][cursor.split_dimension] for i in range(cursor.location_count)])
            # if width is 0 (all the values are the same) don't partition
            # 如果宽度为0(所有值都相同)，则不进行分区
            ...
            cursor.left = KDTree(
                bucket_size=cursor.bucket_size,
...
class KDTreeClustering:
    ...
    def fit(self, X):  # x<class 'tuple'>: (5685, 2)
        # X is an array
        if hasattr(X, 'shape'):
            n_samples = X.shape[0]  # 5685
            dimensions = X.shape[1]  # 2
        else:
            n_samples = len(X)
            dimensions = len(X[0])
        ...
        self.clusters = [
            leave.index_locations for leave in self.kdtree.getLeaves()]
        clusters = [
            cluster.index_locations for cluster in self.kdtree.getLeaves()]
...
if __name__ == '__main__':
    ...
    geolocate.initialize(
        granularity=params.BUCKET_SIZE,
        write=False,
        readText=True,
        reload_init=False,
        regression=False)
    locations = [geolocate.locationStr2Float(
        loc) for loc in params.trainUsers.values()]
