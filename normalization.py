...
def aug_normalized_adjacency(adj):  # 邻接矩阵？
    adj = adj + sp.eye(adj.shape[0])  # tuple-(9475,9475)
    adj = sp.coo_matrix(adj)  # tuple-(9475,9475)
    row_sum = np.array(adj.sum(1))  # （9475,1）
    d_inv_sqrt = np.power(row_sum, -0.5).flatten()  # （9475，）
    d_inv_sqrt[np.isinf(d_inv_sqrt)] = 0.
    d_mat_inv_sqrt = sp.diags(d_inv_sqrt)  # （9475,9475）
